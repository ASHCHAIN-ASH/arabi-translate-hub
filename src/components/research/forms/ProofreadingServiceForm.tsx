...
import {
  ...
  Send, CheckCircle2, Loader2, Upload, CheckCheck
} from 'lucide-react';
...
export const ProofreadingServiceForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.functions.invoke('send-research-service-request', {
        body: {
          serviceTitle: 'التدقيق اللغوي والمراجعة',
  ...
  if (submitted) {
    return (
      ...
      >
        <Card className="p-8 md:p-12 text-center bg-gradient-to-br from-emerald-50/80 to-teal-50/80 dark:from-emerald-950/30 dark:to-teal-950/30 border-0 shadow-2xl backdrop-blur-sm">
          <motion.div
            initial={{ scale: 0 }}
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-emerald-700 dark:text-emerald-400 mb-4">
    ...
    >
      <Card className="p-6 md:p-10 bg-card/95 backdrop-blur-sm shadow-2xl border-0 rounded-3xl overflow-hidden">
        ...
        <div className="relative text-center mb-8">
          ...
          <div className="w-24 h-1 bg-gradient-to-r from-emerald-500 to-teal-500 mx-auto mt-4 rounded-full" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative">
          ...
          >
            ...
            <Input
              ...
              className="h-12 text-base border-2 border-border/50 focus:border-emerald-500 transition-all rounded-xl"
            />
          </motion.div>
          ...
          >
            ...
            <Textarea
              ...
              rows={5}
              className="text-base resize-none border-2 border-border/50 focus:border-emerald-500 transition-all rounded-xl"
              minLength={30}
            />
          ...
          >
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-14 text-lg font-bold bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 hover:opacity-90 shadow-lg transition-all duration-300 rounded-xl"
            >
              {loading ? (
                <>
