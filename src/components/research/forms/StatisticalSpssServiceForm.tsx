...
import {
  ...
  Send, CheckCircle2, Loader2, BarChart3
} from 'lucide-react';
...
export const StatisticalSpssServiceForm = () => {
  ...
  if (submitted) {
    return (
      ...
      >
        <Card className="p-8 md:p-12 text-center bg-gradient-to-br from-purple-50/80 to-indigo-50/80 dark:from-purple-950/30 dark:to-indigo-950/30 border-0 shadow-2xl backdrop-blur-sm">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1, rotate: 360 }}
            transition={{ duration: 0.6, type: "spring" }}
            className="mx-auto w-24 h-24 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-full flex items-center justify-center mb-6 shadow-lg"
          >
            <CheckCircle2 className="w-16 h-16 text-white" />
          </motion.div>
          <h2 className="text-2xl md:text-3xl font-bold text-purple-700 dark:text-purple-400 mb-4">
  ...
  return (
    ...
    >
      <Card className="p-6 md:p-10 bg-card/95 backdrop-blur-sm shadow-2xl border-0 rounded-3xl overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-500/5 via-indigo-500/5 to-blue-500/5 pointer-events-none" />
        ...
        <div className="relative text-center mb-8">
          <motion.div
            ...
            className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-2xl mb-4 shadow-lg"
          >
            <BarChart3 className="w-8 h-8 text-white" />
          </motion.div>
          
          <h2 className="text-2xl md:text-3xl font-bold mb-3 bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
            اطلب الخدمة الآن
          </h2>
          <p className="text-muted-foreground text-base md:text-lg">
            التحليل الإحصائي و SPSS
          </p>
          <div className="w-24 h-1 bg-gradient-to-r from-purple-500 to-indigo-500 mx-auto mt-4 rounded-full" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative">
          ...
          >
            ...
            <Input
              ...
              className="h-12 text-base border-2 border-border/50 focus:border-purple-500 transition-all rounded-xl"
            />
          </motion.div>
          ...
          >
            ...
            <Textarea
              ...
              rows={5}
              className="text-base resize-none border-2 border-border/50 focus:border-purple-500 transition-all rounded-xl"
              minLength={30}
            />
            <p className="text-sm text-muted-foreground mt-2">
              {formData.details.length > 0 && (
                <span className={formData.details.length >= 30 ? 'text-purple-600' : 'text-amber-600'}>
          ...
          >
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-14 text-lg font-bold bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 hover:opacity-90 shadow-lg transition-all duration-300 rounded-xl"
            >
              {loading ? (
                <>
