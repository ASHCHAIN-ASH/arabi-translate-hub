    ...
    <section className="bg-background py-7 sm:py-9" dir="rtl" aria-labelledby="journal-banner-title">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          className="group relative min-h-[220px] overflow-hidden rounded-lg border border-primary/20 bg-deep-violet shadow-strong sm:min-h-[235px]"
        >
          <img
            ...
            width={1280}
            height={720}
            className="absolute inset-0 h-full w-full object-cover object-left transition-transform duration-700 group-hover:scale-[1.025]"
          />
          <div className="absolute inset-0 bg-gradient-to-l from-deep-violet via-deep-violet/95 to-deep-violet/25" aria-hidden="true" />
          <div className="absolute inset-0 academic-grid opacity-10" aria-hidden="true" />

          <motion.div
            animate={reduceMotion ? undefined : { rotate: [0, 8, 0], y: [0, -6, 0] }}
            transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
            className="absolute left-[32%] top-5 hidden h-11 w-11 items-center justify-center rounded-lg border border-deep-violet-foreground/20 bg-deep-violet-foreground/10 text-accent-light backdrop-blur-sm lg:flex"
            aria-hidden="true"
          >
            <FileCheck2 className="h-5 w-5" />
          </motion.div>
          <motion.div
            animate={reduceMotion ? undefined : { rotate: [0, -9, 0], y: [0, 7, 0] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 0.6 }}
            className="absolute bottom-5 left-[39%] hidden h-10 w-10 items-center justify-center rounded-full border border-deep-violet-foreground/20 bg-deep-violet-foreground/10 text-secondary-light backdrop-blur-sm lg:flex"
            aria-hidden="true"
          >
            <LibraryBig className="h-5 w-5" />
          </motion.div>

          <div className="relative z-10 flex min-h-[220px] max-w-3xl flex-col justify-center p-6 sm:min-h-[235px] sm:p-8 lg:p-10">
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              ...
              <Button asChild size="lg" className="group/button shrink-0 gap-2 bg-background text-foreground shadow-medium hover:bg-muted">
                <Link to="/journals">
                  تصفّح دليل المجلات
                  <ArrowLeft className="h-4 w-4 transition-transform duration-300 group-hover/button:-translate-x-1" />
                </Link>
              </Button>
            </div>
          </div>
