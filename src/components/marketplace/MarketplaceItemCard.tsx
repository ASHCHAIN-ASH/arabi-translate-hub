...
import { Loader2, Lock, Sparkles, Check, AlertTriangle } from 'lucide-react';
import { motion } from 'framer-motion';
import { toast } from 'sonner';
import { MarketplaceItem, MarketplaceService } from '@/utils/marketplaceService';
...
export default function MarketplaceItemCard({ item, userXp, userLevel, onPurchased }: Props) {
  ...
  const handlePurchase = async () => {
    setBusy(true);
    try {
      const r = await MarketplaceService.purchase(item.id);
      if (r.success) {
            ...
            >
              {!inStock ? 'نفدت الكمية' :
                !meetsLevel ? `يحتاج مستوى ${item.min_level}` :
                  !canAfford ? `يلزم ${(item.xp_cost - userXp).toLocaleString('ar-SA')} XP إضافية` :
                    'شراء'}
            </Button>
          </CardContent>
        </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent dir="rtl" className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              راجع تفاصيل المنتج قبل التأكيد
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-destructive">
                <span>التكلفة:</span>
                <span className="font-semibold">- {item.xp_cost.toLocaleString('ar-SA')} XP</span>
              </div>
              <div className="flex justify-between pt-2 border-t font-bold">
                <span>الرصيد بعد الشراء:</span>
                <span>{(userXp - item.xp_cost).toLocaleString('ar-SA')} XP</span>
              </div>
            </div>
