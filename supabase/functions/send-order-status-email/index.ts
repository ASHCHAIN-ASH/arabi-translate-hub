...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; direction: rtl !important; text-align: right !important; }
        ...
        .header::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 6px; background: linear-gradient(90deg, #ffd89b 0%, #19547b 100%); }
        .header h1 { margin: 0; font-size: 28px; font-weight: 700; text-align: center !important; }
        .header p { margin: 10px 0 0 0; font-size: 16px; opacity: 0.9; text-align: center !important; }
        .content { padding: 40px 30px; direction: rtl !important; }
        .status-badge { display: inline-block; background-color: ${statusColor}; color: white; padding: 12px 24px; border-radius: 25px; font-weight: bold; font-size: 14px; margin: 20px 0; }
        .order-details { background-color: #f8fafc; border-radius: 8px; padding: 25px; margin: 20px 0; border-right: 4px solid #667eea; }
        .order-details h3 { margin: 0 0 15px 0; color: #374151; font-size: 18px; }
        ...
        .btn { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-weight: 600; font-size: 16px; }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!requestData.clientEmail) {
      return new Response(JSON.stringify({ success: false, error: "No client email provided" }), {
        status: 400,
