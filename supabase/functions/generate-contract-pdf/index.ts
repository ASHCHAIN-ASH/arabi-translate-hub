...
// محتوى عقد قياسي شامل يُستخدم تلقائياً عند غياب البنود
function buildStandardContractBody(contract: any, signature: any): string {
  const total = Number(contract.total_amount || 0);
  const currency = contract.currency || "SAR";
  const money = (n: number) => new Intl.NumberFormat("ar-SA", {
    style: "currency", currency, maximumFractionDigits: 2
  }).format(n || 0);
  const serviceName = contract.service_name || contract.title || "الخدمة الأكاديمية المتفق عليها";
  const clientName = contract.client_full_name || signature?.signer_name || "العميل";
  const deliveryDate = contract.delivery_date
    ? new Date(contract.delivery_date).toLocaleDateString("ar-SA", { year: "numeric", month: "long", day: "numeric" })
    : "حسب الجدول الزمني المتفق عليه بين الطرفين";

  return `
## المادة الثالثة: نطاق الخدمة وموضوع العقد

1. يلتزم **الطرف الأول** (${PLATFORM.legal}) بتقديم الخدمة الأكاديمية المعنونة: **${serviceName}** للطرف الثاني (${clientName})، وفقاً للمواصفات والمتطلبات الأكاديمية المتفق عليها.
2. تشمل الخدمة جميع الأعمال المهنية اللازمة لإنجاز المتطلب الأكاديمي بالجودة المعهودة لدى المنصة، بما في ذلك المراجعة والتدقيق والإخراج الفني.
3. يتعهد الطرف الأول بأن جميع الأعمال أصلية وخالية من الانتحال (Plagiarism Free) ووفق المعايير الأكاديمية المعتمدة.
4. تُقدَّم جميع الخدمات وفق منهجيات بحثية معتمدة، مع توثيق علمي صحيح حسب الأسلوب المطلوب (APA / Harvard / Chicago …).

## المادة الرابعة: قيمة العقد وآلية السداد

1. القيمة الإجمالية المتفق عليها لهذا العقد هي **${money(total)}** (${currency}) شاملة جميع الأعمال الموصوفة في المادة الثالثة.
2. يتم السداد عبر القنوات الإلكترونية المعتمدة على منصة **${PLATFORM.domain}** (بطاقات الدفع، Apple Pay، التحويل البنكي، أو أي وسيلة دفع تفعّلها المنصة).
3. تُعتبر الخدمة قيد التنفيذ فور إتمام السداد بالكامل وصدور **فاتورة ضريبية رسمية** من المنصة.
4. أي تعديلات جوهرية تخرج عن نطاق العمل الأصلي قد تستوجب تكلفة إضافية يتم الاتفاق عليها كتابياً قبل التنفيذ.

## المادة الخامسة: مدة التنفيذ والتسليم

1. يلتزم الطرف الأول بتسليم العمل المطلوب في الموعد التالي: **${deliveryDate}**.
2. يحق للطرف الثاني طلب تمديد المدة كتابياً قبل ميعاد التسليم بثلاثة أيام عمل على الأقل.
3. أي تأخير من جهة الطرف الثاني في تزويد الطرف الأول بالمعلومات أو المتطلبات يُعفي الطرف الأول من التأخير المقابل في التسليم.
4. يتم التسليم إلكترونياً عبر لوحة الطلبات الخاصة بالعميل في المنصة، أو عبر البريد الإلكتروني المسجّل.

## المادة السادسة: المراجعات والتعديلات

1. يحق للطرف الثاني طلب **مراجعتين مجانيتين** على العمل المُسلَّم خلال **سبعة أيام** من تاريخ التسليم النهائي.
2. تكون التعديلات ضمن نطاق العمل الأصلي الموصوف في المادة الثالثة، ولا تشمل إضافة متطلبات جديدة أو تغيير جوهري في موضوع العمل.
3. أي تعديلات تتجاوز ما ذُكر أعلاه أو خارج فترة المراجعة، تخضع لتسعير إضافي يُتفق عليه كتابياً.

## المادة السابعة: حقوق الملكية الفكرية والسرية

1. تنتقل الملكية الكاملة للعمل النهائي إلى الطرف الثاني فور إتمام السداد بالكامل.
2. يحتفظ الطرف الأول بحق عرض اسم الخدمة (دون أي محتوى تفصيلي) ضمن قائمة أعماله المرجعية لأغراض تسويقية.
3. يلتزم الطرف الأول بسرية تامة تجاه جميع البيانات والمعلومات الشخصية للطرف الثاني، ولا يجوز إفشاؤها أو مشاركتها مع أي طرف ثالث.
4. لا يحق للطرف الثاني إعادة بيع العمل أو نشره تحت اسم جهة أخرى دون إذن كتابي مسبق من الطرف الأول.

## المادة الثامنة: الإلغاء وردّ المبالغ

1. يحق للطرف الثاني طلب إلغاء العقد قبل **بدء التنفيذ الفعلي** مقابل استرداد **80%** من المبلغ المدفوع.
2. بعد بدء التنفيذ، يُحسب المبلغ المسترد بناءً على نسبة الإنجاز الفعلية وقت الإلغاء.
3. لا يحق للطرف الثاني المطالبة بأي استرداد بعد تسليم العمل النهائي وقبول التسليم.

## المادة التاسعة: المسؤولية وحدودها

1. لا يُعتبر الطرف الأول مسؤولاً عن أي قرارات أكاديمية أو نتائج تقييم تُتخذ من قِبل الجهة الأكاديمية للطرف الثاني.
2. يتحمّل الطرف الثاني المسؤولية الكاملة عن استخدام المخرجات النهائية وفق ضوابط جامعته/جهته الأكاديمية.
3. تقتصر مسؤولية الطرف الأول التعاقدية على قيمة هذا العقد كحدّ أقصى، ولا يُسأل عن أي أضرار غير مباشرة.

## المادة العاشرة: التواصل والإشعارات

1. يكون التواصل الرسمي بين الطرفين عبر الجوال **${contract.client_phone || "—"}** أو البريد المسجّل في المنصة.
2. تُعتبر جميع الإشعارات والرسائل المرسلة عبر القنوات أعلاه مُستلمة وذات أثر قانوني.
3. يلتزم كل طرف بتحديث بياناته فور تغييرها لضمان استمرار التواصل الفعّال.

## المادة الحادية عشرة: الأحكام العامة وحلّ النزاعات

1. يخضع هذا العقد ويُفسَّر وفق الأنظمة المعمول بها في **${PLATFORM.jurisdiction}**.
2. يبذل الطرفان جهدهما لحلّ أي خلاف ينشأ عن تنفيذ هذا العقد ودياً، فإن تعذّر ذلك يُحال النزاع إلى الجهة القضائية المختصة في المملكة العربية السعودية.
3. يُعتبر التوقيع الإلكتروني على هذا العقد عبر منصة **${PLATFORM.domain}** ملزماً قانونياً للطرفين، ومعتمداً وفق نظام التعاملات الإلكترونية السعودي.
4. حُرِّر هذا العقد إلكترونياً من نسخة واحدة محفوظة في خوادم المنصة، ويُمكن للطرفين تحميل نسخة مطابقة في أي وقت من حساب العميل.

## المادة الثانية عشرة: الإقرار والقبول

أقرّ أنا/**${signature?.signer_name || clientName}** بأنني قرأتُ جميع بنود هذا العقد بعناية، وفهمتُها، وأبديتُ موافقتي الصريحة عليها، وقمتُ بالتوقيع إلكترونياً عبر منصة ${PLATFORM.domain} بكامل أهليتي المعتبرة شرعاً ونظاماً.
`.trim();
}
...
<div class="page">
  ...
  <div class="parties">
    ...
    <div class="party">
      ...
      <table>
        ${partyRow("الاسم", contract.client_full_name || signature?.signer_name || "—")}
        ${partyRow("البريد", contract.client_email || signature?.signer_email || "—")}
