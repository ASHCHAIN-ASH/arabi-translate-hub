...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const inquiry: DocumentTranslationInquiry =body;
    ...
    try {
      ...
      await supabaseAdmin.from("inbox_messages").insert({
        ...
        service_type: data.serviceType ?? 'document-translation',
        source_page: typeof data.sourcePage === "string" ? data.sourcePage : null,
        priority: "high",
        status: "new",
        metadata: { document_type: data.documentType, source_language: data.sourceLanguage, target_language: data.targetLanguage, document_size: data.documentSize, urgency: data.urgency, company: data.company },
      });
    } catch (inboxErr) {
