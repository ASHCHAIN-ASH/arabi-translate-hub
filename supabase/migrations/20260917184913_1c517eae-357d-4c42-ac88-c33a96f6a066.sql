...
BEGIN
  IF NEW.status = 'approved' AND OLD.status IS DISTINCT FROM 'approved' THEN
    ...
    -- Tiered cashback: the higher the top-up, the higher the cashback
    IF NEW.amount >= 5000 THEN v_bonus_pct := 10;
    ELSIF NEW.amount >= 2500 THEN v_bonus_pct := 6;
    ELSIF NEW.amount >= 1000 THEN v_bonus_pct := 3;
    ELSIF NEW.amount >= 500 THEN v_bonus_pct := 1;
    END IF;
