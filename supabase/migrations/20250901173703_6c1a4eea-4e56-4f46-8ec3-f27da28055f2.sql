-- إعادة كتابة دالة verify_admin_login بدون استخدام crypt
-- سنستخدم مقارنة مباشرة مع hash موجود مؤقتاً حتى نصلح التشفير
...
BEGIN
    ...
    -- التحقق من كلمة المرور (مؤقت - نتحقق من كلمة مرور محددة)
    -- هذا مؤقت حتى نصلح التشفير الصحيح
    is_password_valid := (
    ...
    ELSE
        ...
        INSERT INTO public.security_audit_logs (
            ...
            'failed_admin_login',
            COALESCE(admin_record.id, gen_random_uuid()),
            'wrong_password_attempt',
            'high',
            jsonb_build_object(
                'email', COALESCE(admin_record.email, email_input),
                'ip_address', inet_client_addr(),
                'timestamp', now()
            )
        );
